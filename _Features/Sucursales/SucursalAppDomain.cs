    ...
    public class SucursalAppDomain
    {
        public ErrorOr<bool> ValidateSucursal(SucursalDto sucursalDto)
        {
            ...
            if (string.IsNullOrWhiteSpace(sucursalDto.Nombre))
                return Error.Validation("El nombre de la sucursal es obligatorio.");
            if (string.IsNullOrWhiteSpace(sucursalDto.Direccion))
                return Error.Validation("La dirección de la sucursal es obligatoria.");
            ...
            if (sucursalDto.Direccion.Length > 200)
                return Error.Validation("La dirección no puede exceder los 200 caracteres.");
            if (sucursalDto.Latitud ==0 || sucursalDto.Latitud == 0)
                return Error.Validation("La latitud no puede ser 0.");

            if (sucursalDto.Latitud < -90 || sucursalDto.Latitud > 90)
                return Error.Validation("La latitud debe estar entre -90 y 90.");
            if (sucursalDto.Longitud < -180 || sucursalDto.Longitud > 180)
                return Error.Validation("La longitud debe estar entre -180 y 180.");
